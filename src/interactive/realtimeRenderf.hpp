...
namespace realtimeRender{
    ...
    template <class Tptr>
    __host__ void simpleRender(sf::Uint8* pixels, Camera& cam, Tptr targetHolder, BackgroundColor** background, curandState* randState) {
    ...
    while (window.isOpen()) {
        ...
        if (changes) {
            ...
            simpleRender(pixels, camCpy, targetHolder, background, randState);
            frame++;
