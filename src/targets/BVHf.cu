...
__device__ void BVHTree::clear() {
    delete[] nodes;
    delete[] tIdx;
}

__device__ void BVHTree::setNodeBounds(uint nodeIdx) {
...
__device__ void BVHTree::subDivision() {
...
__device__ void BVHTree::simpleSplit(Node* node, int& axis, float& splitPosition) {
