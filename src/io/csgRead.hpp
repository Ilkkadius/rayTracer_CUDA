...
namespace csgRead {
    ...
    __host__ bool findNext(streamHolder sh, std::string& line, uint& linenum);

    __host__ void csgFromFile(const char* path, TargetList** list);
    ...
    __host__ void readTransformation(streamHolder sh, std::string& line, uint& linenum, affine& aff);
