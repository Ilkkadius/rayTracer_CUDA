...
__host__ void realtimeRender::simpleRender(sf::Uint8* pixels, Camera& cam, TargetList** targetPtr, BackgroundColor** background, curandState* randState) {
...
__host__ void realtimeRender::simpleRender(sf::Uint8* pixels, Camera& cam, BVHTree** tree, BackgroundColor** background, curandState* randState) {
    static int tx = 8, ty = 8;
    static dim3 blocks(divup(cam.width, tx), divup(cam.height, ty));
    static dim3 threads(tx, ty);
    ...
    *windowPtr = WindowVectors(cam.window); // Room for optimization here ..
    ...
    completeRender<<<blocks, threads>>>(pixels, cam.width, cam.height, cam.depth, cam.samples, tree, background, windowPtr, randState);
    CHECK(cudaDeviceSynchronize());
    CHECK(cudaFree(windowPtr));
    CHECK(cudaDeviceSynchronize());
}
...
__host__ void realtimeRender::startCamera(Camera& cam, TargetList** targetPtr, BackgroundColor** background, curandState* randState, Vector3D& eye, Vector3D& direction, Vector3D& up) {
    cam.check();
...
__host__ void realtimeRender::startCamera(Camera& cam, BVHTree** tree, BackgroundColor** background, curandState* randState, Vector3D& eye, Vector3D& direction, Vector3D& up) {
    cam.check();
    ...
    while (window.isOpen()) {
        ...
        if (changes) {
            ...
            simpleRender(pixels, camCpy, tree, background, randState);
            frame++;
            window.setTitle("RenderWindow_Frame" + std::to_string(frame) + (speed == 1 ? "f" : ""));
