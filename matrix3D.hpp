...
public:
    ...
    __device__ Matrix(const Matrix& mat) : c1(mat.c1), c2(mat.c2), c3(mat.c3) {}

    __device__ Matrix& operator=(const Matrix& mat) {
        c1 = mat.c1; c2 = mat.c2; c3 = mat.c3;
        return *this;
    }

    __device__ Matrix operator-() const {return Matrix(-c1, -c2, -c3);}
 ...
 * @return Vector3D 
 */
__device__ Vector3D operator*(const Matrix& mat, const Vector3D& vec) {
    return Vector3D(Dot(mat.r1(), vec), Dot(mat.r2(), vec), Dot(mat.r3(), vec));
}

__device__ Matrix operator*(float a, const Matrix& mat) {
    return Matrix(a*mat.c1, a*mat.c2, a*mat.c3);
}

__device__ Matrix operator*(const Matrix& mat, float a) {
    return a*mat;
}
